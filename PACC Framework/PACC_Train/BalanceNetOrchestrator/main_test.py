...
if __name__ == '__main__':
    ...
    # Node 초기화 및 시작
    node = Node(args.local_port, args.parent_address, args.child_addresses, model, train_dataloader, val_dataloader, loss_function, optimizer, args.epochs, args.save_path, retain_graph=True, evaluate_model_func=evaluate_model_TS, train_func=train_TS, batch_size=args.batch_size)
    node.start()
    ######################
    ...
    node = Node(args.local_port, args.parent_address, args.child_addresses, model, train_dataloader, val_dataloader, loss_function, optimizer, args.epochs, args.save_path, train_func=train_CNN, evaluate_model_func=evaluate_model_CNN)
    node.start()
    ######################

    #node.get_metrics()   # 결과 가져오기
    #node.stop_training()  # 학습 중지